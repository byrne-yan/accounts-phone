 ...
 */
Accounts.sendPhoneVerificationCode = function (userId, phone, type=true) {
    ...
    sendRegisterSMS(user.phone.number,verifyObject.code,type,user.username,user.profile?user.profile.name:'');
};
...
Meteor.methods({
    requestPhoneVerification: function (phone, profile) {
        ...
        if(existingUser && existingUser.phone.verified){
        ...
        if (!userId) {
            ...
            if (existingUser) {
                ...
                //console.log("for new phone",phone);
