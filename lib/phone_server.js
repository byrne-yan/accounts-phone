...
Accounts.sendPhoneVerificationCode = function (userId, phone, options) {
    check(options,{
        purpose: Match.Where((x)=>{return x==='register' || x==='bind'}),
    ...
    if(options.purpose==='register')
        sendMessageSync("template:register",user.phone.number,{
            code:verifyObjectcode, minutes:Accounts._options.verificationValidDuration/1000/60});
    ...
    requestPhoneVerification: function (phone, profile) {
        ...
        try{
            if (!userId) {//currently not login
                ...
                Accounts.sendPhoneVerificationCode(userId, phone,{purpose:'register',password:initialPassword});
            }else{
