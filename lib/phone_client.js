 ...
 *      Either a string interpreted as a phone, username or email;
 *      or an object with a single key: `phone` , `email`, `username` or `id`.
 ...
 */
Meteor.loginWithPasswordEx = function (selector, password, callback) {
    var mobileReg = new RegExp(/^(0|86|17951)?(13[0-9]|15[012356789]|18[0-9]|14[57])[0-9]{8}$/);

    if (typeof selector === 'string')
        if (selector.indexOf('@') === -1)
        {
            if(selector.match(mobileReg))
            {
 ...
 * @param {Object} profile -  Optional profile when new user created.
 * @param {Function} [callback] Optional callback. Called with no arguments on success, or with a single `Error` argument on failure.
 */
Accounts.requestPhoneVerification = function (phone, profile, callback) {
    if (!phone || !_.isString(phone))
        throw new Error("Must pass phone");
    if (_.isFunction(profile)) //no profile provided
    {
        callback = profile;
        profile = {};
    }
    if (profile && !_.isObject(profile))
        throw  new Error("profile should be an object");
    if (callback && !_.isFunction(callback))
        throw  new Error("callback should be a function");

    console.log('Accounts.connection.call("requestPhoneVerification",%j,%j,%j', phone, profile, callback);
    Accounts.connection.call("requestPhoneVerification", phone, profile, callback);
};
...
/**
 * @summary Marks the user's phone as verified.  Logs the user in afterwards..
 * @locus Client
...
Accounts.verifyPhone = function (phone, code, callback) {
    ...
    if (!code)
        throw new Error("Need to pass verification code");

    Accounts.callLoginMethod({
        methodName     : 'verifyPhone',
        methodArguments: [phone, code],
        userCallback   : callback});
};
